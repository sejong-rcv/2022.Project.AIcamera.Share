...
class Evaluater:
    ...
    def evaluate_with_depth(self,encoder,depth_decoder,num_workers,data_path,eval_split,height,width,opt):
        ...
        # name_idx=0     
        # model_path="Swin-Transformer-TensorRT/transdssl_pre_16_v2.engine"
        # total_cnt = 0
        # accurate_cnt = 0
        ...
        #                 image= data[("thermal", 0, 0)]
        #                 total_cnt += len(image)
        #                 cur_image = image.numpy()
        #                 batch_images = cur_image
        #                 pred_disp = data["depth_gt"].squeeze().cpu().numpy()
        ...
        #                 pred_disps.append([outputs_trt["output_0"][0][0]])
        #                 # accurate_cnt += image_class_accurate(list(outputs_trt.values())[0], batch_labels)
                        
        #                 # plt.imsave(self.opt.load_weights_folder+"/INPUT/%05d.png"%name_idx,cur_image[0][0])
        #                 # plt.imsave(self.opt.load_weights_folder+"/DISP/%05d.png"%name_idx,outputs_trt["output_0"][0][0],cmap="magma")
        ...
        # print("Evaluation of TRT model on {} , fps: {}".format(total_cnt,
        ...
        with torch.no_grad():
            for idx,data in enumerate(tqdm(dataloader)):
                ...
                gt_depths.append(data["depth_gt"][:,0,:,:].cpu().numpy())
