...
#         torch.distributed.init_process_group(backend='gloo',init_method='env://')
        ...
        if not self.opt.debug:
            wandb.init(project="Pseudo-LIDAR:TransDSSL")
        ...
        if self.opt.model=="DIFF":
            ...
            ##############
            decoder_st_color=self.models["color"].state_dict()
            decoder_st_key_color=list(decoder_st_color.keys())
            ##############
            ...
            for k in pre_keys:
                ...
                if nk=="model.depth_net":
                    ...
                    # if k=="model.depth_net.scratch.output_conv.1.weight":
                    #     import pdb;pdb.set_trace()
                    if k[16:] in decoder_st_key_color and (k!="model.depth_net.scratch.output_conv.1.weight" and k!="model.depth_net.scratch.output_conv.1.bias"):
                        decoder_st_color[k[16:]]=pre["state_dict"][k]
            
            # import pdb;pdb.set_trace()
            self.models["encoder"].load_state_dict(encoder_st)
            self.models["depth"].load_state_dict(decoder_st)
            self.models["color"].load_state_dict(decoder_st_color)
...
#             self.models["depth_t"] =DistributedDataParallel(self.models["depth_t"] )
        ...
        # self.models["color"].cuda()
        # self.parameters_to_train += list(self.models["color"].parameters())
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            if self.opt.distill:
                ...
                # outputs_c_color = self.models["color"](features)


        if self.opt.train_mode=="selfsup":
            ...
            # loss_SIlogs=self.silogloss(outputs_c[("disp",0)], 1/inputs["depth_gt"])*0.01
            loss_depth_ssim=self.compute_reprojection_loss(outputs_c[("disp",0)], 1/inputs["depth_gt"]).mean()
            # import pdb;pdb.set_trace()
            # loss_color_ssim=self.compute_reprojection_loss(outputs_c_color[("color_pred",0)], inputs[("color_aug",0,0)]).mean()
            # loss_color_vgg=self.compute_vgg_loss(outputs_c_color[("color_pred",0)], inputs[("color_aug",0,0)])
            # import pdb;pdb.set_trace()
            losses["loss"]=loss_depth_ssim #+loss_color_ssim+loss_color_vgg
            # outputs_c[("color_pred",0)]=outputs_c_color[("color_pred",0)]
            return outputs_c, losses,outputs_c
...
#             writer.add_scalar("{}".format(l), v, self.step)
        ...
        for j in range(min(4, self.opt.batch_size)):  # write a maxmimum of four images
            for s in self.opt.scales:
                ...
                # wandb.log({"pred_color_{}/{}".format(0, j): wandb.Image(outputs_t[("color_pred", 0)][j].data)})
                ...
                if self.opt.train_mode=="selfsup":
                    ...
                    wandb.log({"disp_gt": wandb.Image(normalize_image(1/inputs["depth_gt"][j]))})
    def save_opts(self):
