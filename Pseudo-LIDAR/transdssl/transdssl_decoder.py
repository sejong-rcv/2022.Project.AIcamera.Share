...
class TRANSDSSLDecoder(TransDSSL_decoder):
    def __init__(
        ...
        head = nn.Sequential(
            nn.Conv2d(features, features // 2, kernel_size=3, stride=1, padding=1),
            nn.Conv2d(features // 2, 3 if self.color else 32, kernel_size=3, stride=1, padding=1),
        )
    ...
    def forward(self, rgb, epoch=0):
        ...
        if not self.color:
            ...
            output[("color_pred",0)]=inv_depth[0]
            output[("color_pred",1)]=inv_depth[1]
            output[("color_pred",2)]=inv_depth[2]
            output[("color_pred",3)]=inv_depth[3]
